...
(function ($) {
    ...
    $(function () {
            ...
            return {
                init: function () {
                    ...
                    $input.on('keydown', async function () {
                        ...
                        var $this = $(this), validate = $this.data('validate'), error = false;
                        if (validate) {
                            ...
                            validate.forEach(function (v) {
                                switch (v) {
                                    case'name':
                                        ...
                                        break;
                                    case'phone':
                                        if ($this.val() && !$this.val().includes('x')) {
                                            $('#submitPhone').attr('disabled', false);
                                        }
                                        else $('#submitPhone').attr('disabled', true);
                                        break;
                                    case'mail':
                                        if ($this.val() && $this.val().match(/\w+@\w+\.\w+/)) {
                                            $('#submitMail').attr('disabled', false);
                                        }
                                        else $('#submitMail').attr('disabled', true);
                                        break;
                                    case'key':
                                        if ($this.val() && $this.val().replace(' ', '').match(/\d{6}/)) {
                                            $this.trigger('blur');
                                        }
                                        break;
                                }
                            ...
                            validate.forEach(function (v) {
                                switch (v) {
                                    case'require':
                                        ...
                                        break;
                                    case'phone':
                                        if ($this.val().includes('x') && !$this.prop('disabled')) {
                                            message += 'Это поле обязательно для заполнения. ';
                                            error = true;
                                        }
                                        buildMessage();
                        ...
                        function reloadDate() {
                            ...
                            if ($window.width() > 990) {
                                ...
                                $dateInputs.filter('[name="fromdaterecent"]').each(function () {
                                    ...
                                    $this.datepicker().data('datepicker').currentDate = today;
                                    $this.datepicker().data('datepicker').update({maxDate: new Date()}).selectDate(today);
                                });
                            } else {
            ...
            return {
                init: function () {
                    ...
                    $('.form_search').on('submit', function (e) {
                        var $this = $(this);
                        e.preventDefault();
                        window.location = '/searchPage?query=' + $this.find('.search-input').val();
                    });
                }
            };
        };
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="/books/' + book.slug + '">' + '       <img src="' + book.image + '">\n' +
                    ...
                    (book.bestseller ? '<div class="Card-ribbon"><span class="Card-ribbonText">Бестселлер</span></div>' : '') +
                    (book.status === 'PAID' ? ' <a class="Card-status" title="Куплена"><img src="/assets/img/icons/check.svg" alt="Куплена"></a>\n' : '') +
                    (book.status === 'ARCHIVED' ? ' <a class="Card-status" title="В архиве"><img src="/assets/img/icons/archive.svg" alt="В архиве"></a>\n' : '') +
                            ...
                            $.ajax({
                                ...
                                complete: function (result) {
                                    if (result.status === 200) {
                                        ...
                                        if (result.responseJSON.count < $('.btn_primary').data('refreshlimit')) {
                        ...
                        switch (data) {
                            case'changedate':
                                if (e.type === 'change') {
                                    ...
                                    var validDate = regex.test($this.val()) && (new Date(date[2] + '-' + date[1] + '-' + date[0]) != 'Invalid Date') && !((date[2] % 4 != 0) && (date[1] == 2) && (date[0] == 29));
                                    if ($this.is('[data-refreshfrom]')) {
                                        if (!validDate && $this.val().length !== 0) {
                                            $this.val($this.data('refreshfrom'));
                                            break;
                                        }
                    ...
                    $('.Comments-addComment form').on('submit', function (e) {
                        ...
                        if (!error) {
                            Login().postData('/bookReview', {
                                bookSlug: $this.data('bookid'),
                                text: $this.find('.Comments-review').val()
                            }, function (result) {
                                if (result.result) {
                                    $this.find('.Comments-successSend').text('Отзыв успешно сохранен');
                                    $this.find('.Comments-review').attr('disabled', true);
                    ...
                    $('[data-likeid]').on('click', function (e) {
                        ...
                        Login().postData('/rateBookReview', data, function (result) {
                            if (result != null && result.result) {
                                ...
                                if (!$this.closest('.Comment-appeal').find('.Rating-title').length) {
                                    $this.closest('.Comment-appeal').append('<span class="Rating-title Rating-title_error"></span>');
                                }
                                $this.closest('.Comment-appeal').find('.Rating-title').text(result.error);
                                setTimeout(function () {
                                    $this.closest('.Comment-appeal').find('.Rating-title').text('');
                                }, 2500);
                            }
                        });
                    });
                    ...
                    $('.Contacts-form').find('form').on('submit', function(e) {
                        ...
                        if (!$this.find('.form-error').length) {
                            ...
                            $.ajax({
                                url: '/message',
                                method: 'post',
                                data: {name: $('#name').val(), mail: $('#mail').val(), subject: $('#topic').val(), text: $('#message').val()},
                                complete: function() {
                                    $this.append('<div class="form-info">Сообщение отправлено</div>');
                                }
                            });
                        }
                    });
                ...
                init: function () {
                    ...
                    $('#submitPhone').on('click', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'),
                            data = {contact: $row.find('.form-input_phone').val(), type: $row.find('.form-input_phone').attr('name')};
                            $row.find('.form-input_phone').trigger('blur');
                        ...
                        postData('/requestContactConfirmation', data, function (result) {
                            if (result.result) {
                                ...
                                $('#phone').next('.form-error').remove();
                                $('#phone').addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                            }
                        })
                    });
                    $('#submitMail').on('click', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'),
                            data = {contact: $row.find('.form-input_mail').val(), type: $row.find('.form-input_mail').attr('name')};
                            $row.find('.form-input_mail').trigger('blur');
                            $this.attr('disabled', true);
                        ...
                        postData('/requestContactConfirmation', data, function (result) {
                            if (result.result) {
                                ...
                                $('#mail').next('.form-error').remove();
                                $('#mail').addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                            }
                        })
                    });
                        ...
                        if (!$this.hasClass('form-input_error')) {
                            ...
                            postData('/approveContact', data, function (result) {
                                ...
                                if (result.result) {
                                    ...
                                    $row.find('.form-input_mail, .form-input_phone').prop('disabled', false);
                                    $but.next('.form-info').remove();
                                    $this.val('');
                                    $this.attr('disabled', false);
                    ...
                    $Registration.find('.form-input').on('change', function () {
                        var $this = $(this), $row = $this.closest('.form-group_row'), $but = $row.find('.form-btn'),
                            ...
                            $butReg = $Registration.find('#registration');
                    ...
                    $Registration.on('submit', function(e) {
                        e.preventDefault();
                        var regButton = $('#registration');
                        if (!regButton.parent().find('.form-info').length) {
                            regButton.attr('disabled', true);
                            var data = {name: $('#name').val(), phone: $('#phone').val(), mail: $('#mail').val()}
                            postData('/register', data, function() {
                                regButton.attr('disabled', false);
                                regButton.text('Вернуться назад');
                                regButton.after('<div class="form-info">Регистрация прошла успешно</div>');
                            });
                        }
                        else if (regButton.parent().find('.form-info').length) {
                            var redirect = document.cookie.match('loginReferer=(.+);?');
                            document.cookie = 'loginReferer=; max-age=0';
                            redirect ? window.location.assign(redirect[1]) : window.location.assign('/profile');
                        }
                    });
                    ...
                    $sendButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            if (!error) {
                                ...
                                postData('/login_sendContact', $this.data('send'), function (result) {
                                    if (result.result) {
                    ...
                    $entryButton.on('click', function (e) {
                        ...
                        if ($this.is('[data-send]')) {
                            ...
                            if (!error) {
                                $sendButton.attr('disabled', true);
                                postData('/login_approveContact', $this.data('send'), function (result) {
                                    if (result.result) {
                    ...
                    $Login.find('#vkAuth').on('click', function () {
            ...
            return {
                init: function () {
                    ...
                    $btnSend.on('click', function (e) {
                        ...
                        if ($this.data('sendstatus') === 'ARCHIVED' && $this.data('check')) {
                        ...
                        } else if ($this.hasClass('Cart-buyAll') && $this.data('sendstatus') === 'PAID') {
                            ...
                            Login().postData('/buy', {
                                ...
                                if (result.result) {
                                    ...
                                    if (result.error === 'Для покупки необходимо авторизоваться') {
                            ...
                            Login().postData('/changeBookStatus', {
                                ...
                                $this.attr('disabled', false);
                                if (result.result) {
                                    $('#cartAmount').text(result.cartAmount);
                                    $('#keptAmount').text(result.keptAmount);
                                    if ($cart.length) {
                                        var price = 0, priceOld = 0, booksId = [];
                                        $this.closest('.Cart-product').remove();
                                        $cart.find('.Cart-product').each(function () {
                                            var $this = $(this);
                                            if ($this.find('.Cart-price_old').text().length === 0) {
                                                price += parseFloat($this.find('.Cart-price:not(.Cart-price_old)').text());
                                                priceOld += parseFloat($this.find('.Cart-price:not(.Cart-price_old)').text());
                                            ...
                                            booksId.push($this.find('[data-sendstatus="CART"], [data-sendstatus="KEPT"]').data('bookid'));
                                        });
                                        $cart.find('.Cart-total .Cart-price').text(price + ' р.');
                                        if (price === priceOld) $cart.find('.Cart-total .Cart-price_old').hide();
                                        else $cart.find('.Cart-total .Cart-price_old').text(priceOld + ' р.');
                                        $cart.find('.Cart-buyAll').data('bookid', booksId.join(', '));
                                        ...
                                        if ($cart.hasClass('Cart_postponed') && $this.hasClass('Cart-buyAll')) {
                    ...
                    $ProductAssesment.find('.Rating input').on('change', function (e) {
                        ...
                        Login().postData('/rateBook', {
                            bookSlug: $this.closest('.Rating').data('bookid'),
                            ...
                            if (result.result) {
                                if (!$this.closest('.Rating').hasClass('Rating_inputClick')) {
                                    $this.closest('.Rating').addClass('Rating_inputClick');
                                }
                                $title.addClass('Rating-title_success').text('Рейтинг успешно изменен');
                            } else {
                                $title.addClass('Rating-title_error').text(result.error);
        ...
        var Profile = function () {
            ...
            return {
                init: function () {
                    ...
                    profileForm.find('#name, #mail, #phone').on('keydown', async function () {
                        await new Promise((resolve) => setTimeout(resolve, 1));
                        var $this = $(this);
                    ...
                    profileForm.find('#changeName').on('click', function() {
                        var $this = $(this);
                        if(!$this.closest('.form-group').find('.form-error').length) {
                            $('#name').attr('disabled', true);
                            $this.attr('disabled', true);
                            var data = {name: $('#name').val()};
                            postData('/editProfile', data, function() {
                                $this.after('<div class="form-info">Имя успешно изменено</div>');
                            });
                        }
                    });
                    profileForm.find('#changeMail, #changePhone').on('click', function() {
                        var $this = $(this);
                        var group = $this.closest('.form-group');
                        var contact = group.find('#mail, #phone');
                        contact.trigger('blur');
                        if(!group.find('.form-error').length) {
                            var type = contact.attr('id') === 'mail' ? 'mail' : 'phone';
                            contact.attr('disabled', true);
                            $this.attr('disabled', true);
                            var data = {contact: contact.val(), type: type};
                            postData('/requestContactConfirmation', data, function(result) {
                                if (result.result) {
                                    group.find('#mailCode, #phoneCode').show();
                                    var successMessage = type === 'mail' ?
                                        'Вам направлен код подтверждения в письме на почту' : 'Вам направлен код подтверждения по SMS';
                                    $this.after('<div class="form-info">' + successMessage + '</div>');
                                } else {
                                    contact.attr('disabled', false);
                                    $this.attr('disabled', false);
                                    contact.addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                                }
                            });
                        }
                    });
                    profileForm.find('#mailCode, #phoneCode').on('blur', function() {
                        var $this = $(this);
                        var group = $this.closest('.form-group');
                        if(!group.find('.form-error').length) {
                            group.find('.form-info').remove();
                            var contact = group.find('#mail, #phone');
                            $this.attr('disabled', true);
                            var data = {contact: contact.val(), code: $this.val()};
                            postData('/approveContact', data, function(result) {
                                $this.hide();
                                if (result.result) {
                                    postData('/editProfile', {contact: contact.val()}, function() {
                                        group.find('button').after('<div class="form-info">Код успешно подтвержден</div>');
                                    });
                                } else {
                                    $this.val('');
                                    $this.attr('disabled', false);
                                    contact.attr('disabled', false);
                                    group.find('#changeMail, #changePhone').attr('disabled', false);
                                    contact.addClass('form-input_error').after('<div class="form-error">' + result.error + '</div>');
                                }
                            });
                        }
                    ...
                    $('form[action*="/logout"]').on('click', function(){
                        $(this).submit();
                    });
                }
            };
                                ...
                                if ($(slick.$slides[slick.$slides.length - 1]).hasClass('slick-active')) {
                                    if ($(slick.$slider).data('load')) {
                                        ...
                                        function sendData(address) {
                                            getData(address, {
                                                offset: function () {
                                                    var offset = $(slick.$slider).data('loadoffset') + 1;
                                                    $(slick.$slider).data('loadoffset', offset);
                ...
                init: function () {
                    ...
                    $('[href*="download"]').on('click', function(e) {
                        ...
                        $.ajax({
                            ...
                            error: function() {
                                $('#myPopup_close').prev('.form-error').remove();
                                $('#myPopup_close').before('<div class="form-error">Книгу можно скачать не более 3 раз</div>');
                                setTimeout(function() {
                                    $('#myPopup_close').prev('.form-error').remove();
                                }, 2500);
                            }
            ...
            return {
                init: function () {
                    ...
                    $('form.Topup').on('submit', function (e) {
                        ...
                        if (sum == $('#sum').val() && sum > 0) {
                            $this.find('button').attr('disabled', true);
                            getData('/payment', {sum: $('#sum').val()}, function (result) {
                                if (result.result) {
                                    window.location.assign(result.redirect);
                                } else {
                                    $this.find('.form-error').remove();
                                    $this.find('input').removeClass('form-textarea_error');
                                    $this.find('button').after('<div class="form-error">' + result.error + '</div>');
                                    $this.find('button').attr('disabled', false);
                                }
                            });
                        } else {
                            $this.find('.form-error').remove();
                            $this.find('input').addClass('form-textarea_error');
                            $this.find('.Topup-group').after('<div class="form-error">Введите целое число</div>');
                        }
                    });
                }
            };
        ...
        var Transactions = function () {
            ...
            var transactionTemplate = function (transaction) {
                ...
                return '<tr>\n' + '                      <td>' +
                    ...
                    formatNumber(date.getMinutes()) + '                      </td>\n' + '                      <td><span class="Transactions_textSuccess">\n' + '                          <strong>' + (transaction.value > 0 ? '+' : '') + transaction.value + ' р.\n' + '                          </strong></span>\n' + '                      </td>\n' + '                      <td>' + transaction.description + '                      </td>\n' + '                    </tr>';
            }
                ...
                $.ajax({
                    ...
                    complete: function (result) {
                        if (result.status === 200) {
                            ...
                            $(window).scrollTop(scroll);
                            if (result.responseJSON.transactions.length < $('.Transactions-get').data('transactionlimit')) {
                                $('.Transactions-footer_center').hide();
                            }
                        } else {
                            alert('Ошибка ' + result.status);
                        }
            ...
            return {
                init: function () {
                    $('.Transactions-get').on('click', function (e) {
                        var $this = $(this);
                        e.preventDefault();
                        var offset = $this.data('transactionoffset') + 1;
                        $this.data('transactionoffset', offset);
        ...
        $(document).ready(function() {
            var locales = $('#locales').val($('html').attr('lang'));
