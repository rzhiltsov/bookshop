...
public class GenresPageController {
    ...
    public String selectedGenrePage(@PathVariable String slug, Model model) {
        ...
        model.addAttribute("currentGenre", path.isEmpty() ? new Genre() : path.pollLast());
        model.addAttribute("genrePath", path);
        model.addAttribute("books", bookService.getBooksByGenre(slug, 0, 20));
        return "/genres/slug";
    }
    ...
    public ObjectNode loadBooksByGenre(@PathVariable String slug, int offset, int limit) {
        ...
        List<Book> resultList = bookService.getBooksByGenre(slug, offset, limit);
