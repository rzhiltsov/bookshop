...
public class PostponedPageController {
    ...
    @Autowired
    public PostponedPageController(BookService bookService, AuthorService authorService, UserService userService) {
    ...
    public String postponedPage(Model model, HttpServletRequest request) {
        ...
        if (SecurityContextHolder.getContext().getAuthentication() instanceof AnonymousAuthenticationToken) {
            ...
            bookList = data.getOrDefault("KEPT", List.of()).stream().map(bookService::getBookEntityBySlug)
            ...
            bookList = userEntity.getBooks().stream().map(bookService::createBook)
                    .filter(book -> book.getStatus().equals("KEPT")).toList();
        }
